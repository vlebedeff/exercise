...
request_count = gets.strip.to_i
request_count.times do
  request = gets.strip
  if request.start_with?('+')
    value = request[2..-1].to_i
    s = s.insert((value + x) % M)
  elsif request.start_with?('-')
    value = request[2..-1].to_i
    s = s.delete((value + x) % M)
  elsif request.start_with?('?')
    value = (request[2..-1].to_i + x) % M
    s = s.find(value).splay
    puts (s.empty? || s.node.value != value) ? 'Not Found' : 'Found'
  elsif request.start_with?('s')
    l, r = request[2..-1].split(' ').map { |i| (i.to_i + x) % M }
    x = s.sum(l..r)
    puts x
  end
end
